...
### アカウント削除（退会）
- 登録ユーザーが自身のアカウントを完全に削除できる（ストア審査の必須要件）
- 設定画面から退会を開始し、確認画面で本人確認のためパスワード再認証を必須とする
- 退会実行時、以下を完全削除する（復元不可）
  - プロフィール（profiles）
  - 投稿・生成済みコンテンツ（image_contents / music_contents）
  - いいね（likes）・コメント（comments）
  - AI 利用ログ（ai_usage_logs）
  - Storage 上の生成ファイル（画像）
  - Supabase Auth のユーザーアカウント
- DB レコードは Auth ユーザー削除に伴い ON DELETE CASCADE で連鎖削除される
- Storage ファイルは Edge Function 内で明示的に削除する
- 削除完了後はローカルセッションを破棄し、未ログイン状態に戻す
...
| 設定画面 | Tab2 | アカウント情報・課金・ログアウト・退会 |
| 退会確認画面 | プッシュ遷移（設定内） | 削除内容の警告・パスワード再認証・退会実行 |
